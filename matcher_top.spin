...
VAR  
  ...
  byte presetindex,nextselectioncntr,nextselectioncntrmax
...
PRI autoselect | nextindex
  if presetflag[presetindex] =< 0 or (nextselectioncntr=<0 and getpower=<powerpreset[presetindex] and getreflected=>reflectedpreset[presetindex]) 
    nextselectioncntr:=nextselectioncntrmax
    ...
    'waitcnt(cnt+autoselectmagnetizationdelay)
  else
    nextselectioncntr:=(nextselectioncntr-1)#>0
...
PRI execute | togglecounter,cogEEPROM,cogPst,cogShunt,cogBallast,ShuntCurrent,BallastCurrent,cogDacP16,DacP16Value,cogDacP17, DacP17Value, directdecrement,direct,directincrement,reflecteddecrement,reflected,reflectedincrement,i,j,p,r
  ...
  pst.newline
  togglecounter := eeprom.readtogglecntr
  nextselectioncntrmax:=((eeprom.readnextselectioncntrmax)#>1)<#20
  nextselectioncntr:=0
  pst.str(string("Current state = "))
  pst.dec(togglecounter)
  pst.str(string(", autoselect delay = "))  
  pst.dec(nextselectioncntrmax)
  pst.newline
  presetindex:=eeprom.readpresetindex//presetindexmax1
  repeat i from 0 to presetindexmax
    presetflag[i]:=eeprom.readpresetflag(i)
    shuntpreset[i]:=eeprom.readshuntpreset(i)
    ballastpreset[i]:=eeprom.readballastpreset(i)
    powerpreset[i]:=eeprom.readpowerpreset(i)    
    reflectedpreset[i]:=eeprom.readreflectedpreset(i)    
    ...
    if pst.RxCount > 0
      ...
      case symbol
        ...
        "R": reflectedpreset[presetindex]:=(reflectedpreset[presetindex]+100)//6001
             'eeprom.savereflectedpreset(presetindex,reflectedpreset[presetindex])
        "a": togglecounter &= 2
             eeprom.savetogglecntr(togglecounter)
             iterationcounter:=0
        ...
        "G": togglecounter |= 2
             ...
             pst.clearend
        "N": nextselectioncntrmax:=(nextselectioncntrmax+1)<#20
             eeprom.savenextselectioncntrmax(nextselectioncntrmax)
             pst.position(0,menupositionN)
             pst.str(string(" Select delay counter =  "))
             pst.dec(nextselectioncntrmax)
             pst.clearend
        "n": nextselectioncntrmax:=(nextselectioncntrmax-1)#>1
             eeprom.savenextselectioncntrmax(nextselectioncntrmax)
             pst.position(0,menupositionN)
             pst.str(string(" Select delay counter =  "))
             pst.dec(nextselectioncntrmax)
        ...
        "?":
          ...
          pst.newline
          pst.str(string(" N / n - INCREASE / decrease selection delay counter"))
          pst.newline
          pst.str(string(" f - Iterate through entire shunt & ballast ranges to find optimal point"))
          pst.newline
          pst.str(string(" @ - Switch preset point if matching conditions require"))
          pst.newline
          pst.str(string(" # - Reboot"))
          pst.newline
          pst.str(string(" ? - Print this help page"))
          pst.newline
